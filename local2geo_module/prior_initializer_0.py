...
# Relative lower-bound scales: compact, gauche-like, extended.
TORSION_ANCHOR_SCALES = (0.65, 0.82, 1.0)
NUM_TORSION_ANCHORS = len(TORSION_ANCHOR_SCALES)
...
class PriorGeometryInitializer(nn.Module):
    ...
    @staticmethod
    def _soft_reach(path_mass: torch.Tensor) -> torch.Tensor:
        """Continuous probability-like map from non-negative walk mass."""
        return -torch.expm1(-path_mass.clamp_min(0.0))
    ...
    def bounds(
        ...
        del atom_mask, heavy  # already encoded by pair_mask; kept in the API.
        ...
        anchor_scales = long_range.new_tensor(TORSION_ANCHOR_SCALES)
        anchor_lower = torch.maximum(
            excluded.unsqueeze(-1),
            saturating.unsqueeze(-1) * anchor_scales,
        )
        if torsion_logits is None:
            fixed_logits = long_range.new_tensor(
                (0.0, 0.0, self.anchor_prior_strength)
            )
            torsion_mixture = torch.softmax(fixed_logits, dim=-1)
        else:
    ...
    def _stress(
        ...
        residual = bounds["target_weight"] * (distance - bounds["target"])
        ...
        energy = (
            0.5 * bounds["target_weight"]
            * (distance - bounds["target"]).square()
