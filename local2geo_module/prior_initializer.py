...
# Maximum number of covalent bonds an element normally forms. Used only to
# damp the aggregate weight of phantom edges, so the values are deliberately
# generous: capping too tightly would penalise legitimate hypervalent centres.
DEGREE_CAPS = {
    1: 1, 5: 3, 6: 4, 7: 4, 8: 2, 9: 1,
...
class PriorGeometryInitializer(nn.Module):
    ...
    def __init__(
        ...
        num_hops: int = 12,
        hop_chunk: int = 16,
        hop_scale: float = 1.00,
        hop_saturation: float = 30.0,
        ...
        self.valence_power = valence_power
        ...
        degree_cap = torch.full((119,), 4.0, dtype=torch.float32)
        for number, cap in DEGREE_CAPS.items():
            degree_cap[number] = float(cap)
        self.register_buffer("degree_cap_table", degree_cap)
    ...
    def _soft_hop_distance(
        ...
        """Differentiable expected graph distance by max-product relaxation.

        Reachability must be propagated in the (max, x) semiring rather than
        by powers of the adjacency matrix. Matrix powers count *walks*, and
        walk counts explode combinatorially with back-and-forth traversal, so
        `1 - exp(-q^k)` saturates to one for every pair after three or four
        steps: the estimate then stops growing and every distant pair collapses
        to the same value, leaving no long-range expansion force at all.
        Tracking the single most reliable path instead keeps the estimate
        monotone in true graph distance for as many hops as are iterated.
        """
        batch, atoms, _ = q.shape
        reach = q.clamp(0.0, 1.0)
        hop = reach
        previous = reach
        chunk = max(1, min(atoms, self.hop_chunk))
        for order in range(2, self.num_hops + 1):
            parts = []
            for start in range(0, atoms, chunk):
                stop = min(start + chunk, atoms)
                extended = (
                    previous[:, start:stop, :, None] * q[:, None, :, :]
                ).max(dim=2).values
                parts.append(extended)
            reach = torch.maximum(previous, torch.cat(parts, dim=1))
    ...
    def _bond_targets(
        ...
        degree_caps: torch.Tensor,
        pair_mask: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Expected 1--2 length and valence-normalised bond probability.

        A dense soft support has no threshold, so every atom carries a small
        residual bond probability to every other atom. Individually these are
        negligible, but an atom in a thirty-heavy-atom molecule has of order
        thirty of them and their aggregate pull can exceed a real bond. The
        effect is worst for hydrogen, which has a single genuine partner and
        the lowest tolerance for competition.

        Damping the whole row once its total bond mass exceeds the element's
        normal valence removes the aggregate without introducing a threshold:
        the ratio between any two edges of the same atom is preserved exactly,
        so a genuinely uncertain edge keeps its gradient, while a row bloated
        by phantoms is scaled back as a whole. Atoms whose predicted degree is
        already sensible are left untouched.
        """
        bonded = probabilities[..., 1:]
        raw = bonded.sum(dim=-1) * pair_mask
        sharpened = raw.pow(self.valence_power)
        total = sharpened.sum(dim=-1)
        scale = (degree_caps / total.clamp_min(1e-8)).clamp_max(1.0)
        q = sharpened * torch.minimum(scale[:, :, None], scale[:, None, :])
        ...
        return length.clamp_min(0.5), q, raw
    ...
    def bounds(
        ...
        bond_length, q, raw_q = self._bond_targets(
            probabilities, covalent_radii, degree_caps, pair_mask_f
        )
        cos_theta = (
        ...
        # Exclusion must be gated on a *saturating* function of the bond
        # probability, not on 1 - q. A pair that is merely probably bonded
        # (q = 0.65, which is all a softmax over thirty candidate parents
        # gives a hydrogen at a modest logit margin) would otherwise retain a
        # third of a full excluded-volume push and be driven to the van der
        # Waals separation of its own bonding partner, tearing the bond apart.
        bonded_gate = self._soft_reach(bond_membership)
        unbonded = 1.0 - bonded_gate
        one_three_membership = unbonded * self._soft_reach(two_hop_mass)
        ...
        # The graph-distance expansion term must act on the heavy skeleton
        # only. Hydrogens are terminal: a long-range push between two of them
        # has no torsion to relieve it, so the sole remaining freedom is to
        # stretch the C-H bond. Pairs involving hydrogen therefore keep plain
        # excluded volume, and their positions follow the skeleton instead.
        skeleton_pair = heavy[:, :, None] * heavy[:, None, :]
        long_range = torch.maximum(
            skeleton_pair * saturating, excluded
        ) * pair_mask_f
    ...
    def forward(
        ...
        bounds = self.bounds(
            ...
            covalent_radii,
            vdw_radii,
            self.degree_cap_table[atomic_numbers],
