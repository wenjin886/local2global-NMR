...
def collate_shift_samples(samples: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    max_h = max(int(sample["h_shifts"].numel()) for sample in samples)
    max_c = max(int(sample["c_shifts"].numel()) for sample in samples)
    ...
    equivalence = torch.full((batch_size, max_atoms), -1, dtype=torch.long)
    h_prediction_mask = torch.zeros((batch_size, max_atoms), dtype=torch.bool)
    h_targets = torch.zeros((batch_size, max_h), dtype=torch.float32)
    h_target_mask = torch.zeros((batch_size, max_h), dtype=torch.bool)
    c_targets = torch.zeros((batch_size, max_c), dtype=torch.float32)
    c_target_mask = torch.zeros((batch_size, max_c), dtype=torch.bool)
    for row, sample in enumerate(samples):
        atoms = int(sample["atomic_numbers"].numel())
        h_count = int(sample["h_shifts"].numel())
        c_count = int(sample["c_shifts"].numel())
        atomic_numbers[row, :atoms] = sample["atomic_numbers"]
        positions[row, :atoms] = sample["positions"]
        atom_mask[row, :atoms] = True
        equivalence[row, :atoms] = sample["equivalence_classes"]
        h_prediction_mask[row, :atoms] = sample["h_prediction_mask"]
        h_targets[row, :h_count] = sample["h_shifts"]
        h_target_mask[row, :h_count] = True
        c_targets[row, :c_count] = sample["c_shifts"]
        c_target_mask[row, :c_count] = True
    return {
        ...
        "h_prediction_mask": h_prediction_mask,
        "h_targets": h_targets,
        "h_target_mask": h_target_mask,
        "c_targets": c_targets,
        "c_target_mask": c_target_mask,
    }
